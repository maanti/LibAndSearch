...
public interface SourcesRepository extends JpaRepository<Sources,Integer> {
    ...
    @Query(value = "((select id, name, author, publisher, year, type, link, path, tags, similarity(author, :author) similarity from Sources where similarity(author, :author)>0.1) union (select id, name, author, publisher, year, type, link, path, tags, similarity(author, :author) similarity from Sources where lower(author) like concat('%', :author, '%'))) order by similarity desc", nativeQuery = true)
